...
namespace GillSoft.ExpressionEvaluator
{
    ...
    public static class ExtensionMethods
    {
        ...
        internal static Exception CreateException(string token, string message)
        {
            var exception = string.Format("ERROR: {0} in \"{1}\"", message, token);
            var res = new Exception(exception);
            return res;
        }
        ...
        internal static void InvokeHandler<T>(this object sender, Action<T> handler, T eventArgs)
        {
            var handlerTemp = handler;
            if (handlerTemp != null)
            {
                handlerTemp(eventArgs);
            }
        }
